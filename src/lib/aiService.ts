...
- OUTPUT: Pure XML SVG code ONLY. JSON key "svgs" containing the array of strings. IMPORTANT: Your entire response must be a single JSON object. No conversational filler.`;
  ...
  // Gemini logic
  const keys = Array.isArray(apiKeys) ? apiKeys.filter(k => k.trim() !== "") : (apiKeys ? [apiKeys] : []);
  ...
  // Parallel Execution Logic for "Faster" generation
  if (provider === "gemini" && keys.length > 1 && count > 1) {
    const variationsPerKey = Math.ceil(count / keys.length);
    let remainingCount = count;
    const tasks: Promise<string[]>[] = [];

    for (let i = 0; i < keys.length; i++) {
        const currentBatchCount = Math.min(variationsPerKey, remainingCount);
        if (currentBatchCount <= 0) break;
        
        const apiKey = keys[i];
        tasks.push((async () => {
            let attempt = 0;
            const innerMaxRetries = 2;
            
            while (attempt <= innerMaxRetries) {
                try {
                    ...
                    const response = await ai.models.generateContent({
                        ...
                        contents,
                        config: {
                            systemInstruction: systemInstruction.replace(`EXACTLY ${count}`, `EXACTLY ${currentBatchCount}`) + "\nReturn ONLY the JSON. No preamble.",
                            temperature: 0.9,
    ...
    throw new Error("All nodes failed to generate output. Please check your API keys or wait a few moments.");
  }
  ...
  while (retryCount <= maxRetries) {
    try {
      // Key Rotation for single stream: try a random key if multiple provided but count is 1
      const selectedKey = keys.length > 0 ? keys[Math.floor(Math.random() * keys.length)] : undefined;
      const ai = selectedKey ? new GoogleGenAI({ apiKey: selectedKey }) : defaultGeminiAi;
